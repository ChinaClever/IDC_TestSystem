#include "rset_swcyclewid.h"
#include "ui_rset_swcyclewid.h"

RSet_SwCycleWid::RSet_SwCycleWid(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RSet_SwCycleWid)
{
    ui->setupUi(this);
...
RSet_SwCycleWid::~RSet_SwCycleWid()
{
    delete ui;
}

void RSet_SwCycleWid::initwid()
{
    ...
    for(int i=0; i<24; ++i) {
        mWid[i] = new RSet_SwCycleItemWid(wid[i]);//24个输出位控件QWidget
        mWid[i]->init(i);
    }
 ...
 */
void RSet_SwCycleWid::updateTextSlot(QString str)
{
    ui->textEdit->append(str);
}

void RSet_SwCycleWid::on_checkBox_clicked(bool checked)
{
    for(int i=0; i<24; ++i) {
 ...
 */
void RSet_SwCycleWid::on_radioButton_clicked(bool checked)
{
    if(checked)
    {
 ...
 * @return oid 对应Oid节点
 */
QString RSet_SwCycleWid::getOid(int i)
{
 ...
 */
void RSet_SwCycleWid::snmpAllOpenOrCloseCase(int& mtimers)
{
 ...
 */
void RSet_SwCycleWid::snmpOtherCase(int& mtimers ,bool end)
{
    if(mtimers != 0)
    {
        sSnmpSetCmd cmd;
 ...
 */
void RSet_SwCycleWid::sendSnmp(int& mtimers)
{
 ...
 * @param [in&out]mtimers 只能是全开0和全关1
 */
void RSet_SwCycleWid::rtuAllOpenOrCloseCase(int& mtimers)
{
    for(int i = 0 ; i < mSelect.size() ; ++i)
     {
        sRtuSetCmd cmd;
 ...
 * @param [in]end 其他一般情况true ，灭掉最后一个输出位false
 */
void RSet_SwCycleWid::rtuOtherCase(int& mtimers ,bool end)
{
    if(mtimers != 0)
    {
        sRtuSetCmd cmd;
 ...
 * @param [in&out]mtimers [0,1]或者[0,mSelect.size()]启动发送命令线程次数
 */
void RSet_SwCycleWid::sendRtu(int& mtimers)
{
    if(ui->comboBox->currentIndex())
    {//逐个亮灭
        if(mtimers == mSelect.size())
 ...
 * @param [in&out]mtimers [0,1]或者[0,mSelect.size()]启动发送命令线程次数
 */
void RSet_SwCycleWid::produceCmd(int& mtimers)
{
 ...
 */
void RSet_SwCycleWid::updateSlot()
{
    produceCmd(mCaseCount);
}

void RSet_SwCycleWid::startSend()
{
    mSelect.clear();
    for(int i = 0 ; i < 24 ; ++i )
    {
...
void RSet_SwCycleWid::on_startBtn_clicked()
{
    if(mStartOrStop)
    {
        startSend();
 ...
 */
void RSet_SwCycleWid::updateCycleCount()
{
    mCaseCount = 0;
    ui->textEdit->clear();
    mCycleCount++;
