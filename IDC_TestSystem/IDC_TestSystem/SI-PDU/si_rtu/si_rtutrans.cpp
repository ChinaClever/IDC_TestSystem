...
SI_RtuTrans *SI_RtuTrans::bulid(QObject *parent)
{
    static SI_RtuTrans* sington = NULL;
    if(sington == NULL) {
        sington = new SI_RtuTrans(parent);
    }
    return sington;
}
 ...
 * @param serial
 */
void SI_RtuTrans::init(SerialPort *serial)
{
    mSerial = serial;
}
 ...
 * @return true
 */
bool SI_RtuTrans::sentSetCmd(int addr, int reg, ushort value, int msecs)
{
    bool ret = false;
...
void SI_RtuTrans::dataUnit(int i, SI_sDataUnit &rtu, sDataUnit &data)
{
...
void SI_RtuTrans::devLine(SI_RtuRecvLine &rtuData, sDevData &data)
{
...
void SI_RtuTrans::envData(SI_RtuRecvLine &rtuData, sEnvData &data)
{
    data.envNum = 1;
...
void SI_RtuTrans::devData(SI_Rtu_Recv *pkt, sDataPacket *packet)
{
    packet->id = pkt->addr;
...
int SI_RtuTrans::transData(int addr, int line, SI_Rtu_Recv *pkt, int msecs)
{
    uchar offLine = 0;
 ...
 * @param line
 */
int SI_RtuTrans::transData(int addr, int line, sDataPacket *pkt, int msecs)
{
...
QByteArray SI_RtuTrans::getSentCmd()
{
    QByteArray array;
...
QByteArray SI_RtuTrans::getRecvCmd()
{
    QByteArray array;
