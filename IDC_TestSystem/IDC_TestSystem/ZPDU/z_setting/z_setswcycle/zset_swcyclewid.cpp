 ...
 */
void ZSet_SwCycleWid::snmpAllOpenOrCloseCase(int& mtimers)
{
 ...
 * @param [in&out]mtimers 只能是全开0和全关1
 */
void ZSet_SwCycleWid::rtuAllOpenOrCloseCase(int& mtimers)
{
    for(int i = 0 ; i < mSelect.size() ; ++i)
     {
        sRtuSetCmd cmd;
...
void ZSet_SwCycleWid::startSend()
{
    mSelect.clear();
    for(int i = 0 ; i < 24 ; ++i )
    {
