#include "mv1set_swcyclewid.h"
#include "ui_mv1set_swcyclewid.h"
MV1Set_SwCycleWid::MV1Set_SwCycleWid(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MV1Set_SwCycleWid)
{
    ui->setupUi(this);
    //mReg = 1003;//MPDU首位输出位寄存器地址
    mReg = MV1_RtuReg_OutputSw;//MPDU首位输出位寄存器地址
    initwid();
}

MV1Set_SwCycleWid::~MV1Set_SwCycleWid()
{
    delete ui;
}

void MV1Set_SwCycleWid::initwid()
{
    ...
    for(int i=0; i<24; ++i) {
        mWid[i] = new MV1Set_SwCycleItemWid(wid[i]);//24个输出位控件QWidget
        mWid[i]->init(i);
    }
 ...
 */
void MV1Set_SwCycleWid::updateTextSlot(QString str)
{
    ui->textEdit->append(str);
}

void MV1Set_SwCycleWid::on_checkBox_clicked(bool checked)
{
    for(int i=0; i<24; ++i) {
 ...
 */
void MV1Set_SwCycleWid::on_radioButton_clicked(bool checked)
{
    if(checked)
    {
 ...
 */
void MV1Set_SwCycleWid::snmpAllOpenOrCloseCase(int& mtimers)
{
 ...
 */
void MV1Set_SwCycleWid::snmpOtherCase(int& mtimers ,bool end)
{
    if(mtimers != 0)
    {
        sSnmpSetCmd cmd;
 ...
 */
void MV1Set_SwCycleWid::sendSnmp(int& mtimers)
{  
 ...
 * @param [in&out]mtimers 只能是全开0和全关1
 */
void MV1Set_SwCycleWid::rtuAllOpenOrCloseCase(int& mtimers)
{
    for(int i = 0 ; i < mSelect.size() ; ++i)
     {
        sRtuSetCmd cmd;
 ...
 * @param [in]end 其他一般情况true ，灭掉最后一个输出位false
 */
void MV1Set_SwCycleWid::rtuOtherCase(int& mtimers ,bool end)
{
    if(mtimers != 0)
    {
        sRtuSetCmd cmd;
 ...
 * @param [in&out]mtimers [0,1]或者[0,mSelect.size()]启动发送命令线程次数
 */
void MV1Set_SwCycleWid::sendRtu(int& mtimers)
{
    if(ui->comboBox->currentIndex())
    {//逐个亮灭
        if(mtimers == mSelect.size())
 ...
 * @param [in&out]mtimers [0,1]或者[0,mSelect.size()]启动发送命令线程次数
 */
void MV1Set_SwCycleWid::produceCmd(int& mtimers)
{
 ...
 */
void MV1Set_SwCycleWid::updateSlot()
{
    produceCmd(mCaseCount);
}

void MV1Set_SwCycleWid::startSend()
{
    mSelect.clear();
    for(int i = 0 ; i < 24 ; ++i )
    {
...
void MV1Set_SwCycleWid::stopSend()
{
...
void MV1Set_SwCycleWid::on_startBtn_clicked()
{
    if(mStartOrStop)
    {
        startSend();
 ...
 */
void MV1Set_SwCycleWid::updateCycleCount()
{
    mCaseCount = 0;
    ui->textEdit->clear();
    mCycleCount++;
