...
void SiDbThresholds::createTable()
{
    QString cmd =
...
bool SiDbThresholds::insertItem(SiDbThresholdItem &item)
{
    bool ret = false;
    item.id = maxId()+1;
...
bool SiDbThresholds::modifyItem(const SiDbThresholdItem &item, const QString &cmd)
{
    bool ret = false;
    QSqlQuery query;
    query.prepare(cmd);
...
void SiDbThresholds::selectItem(QSqlQuery &query, SiDbThresholdItem &item)
{
