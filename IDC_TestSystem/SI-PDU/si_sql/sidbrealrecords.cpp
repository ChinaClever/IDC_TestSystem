...
void SiDbRealRecords::createTable()
{
    QString cmd =
...
bool SiDbRealRecords::insertItem(SiDbRealRecordItem &item)
{
    bool ret = false;
    item.id = maxId()+1;
...
bool SiDbRealRecords::modifyItem(const SiDbRealRecordItem &item, const QString &cmd)
{
    bool ret = false;
    QSqlQuery query;
    query.prepare(cmd);
...
void SiDbRealRecords::selectItem(QSqlQuery &query, SiDbRealRecordItem &item)
{
